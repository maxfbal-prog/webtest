...
$(document).on("submit", "#frmNewRecordForm", function () {
    ...
    // Must return false, or else submitting form results in reloading the page
    return false;
});
...
function checkRecordForm() {
    ...
    if ($("#txtBP").val() != "" &&
       $("#datExamDate").val() != "" &&
        new Date($("#datExamDate").val()) <= new Date() 
        // FIX THIS, PAGE 147
    ) {
        return true;
    }
        ...
        for (var i = 0; i < tbRecords.length; i++) {
            ...
            $("#tblRecords tbody").append(
                ...
                "<a href='#' data-inline='true' data-mini='true' data-role='button' "+
                    "class='btnEdit' data-record-index='" + i + "'>Edit</a>" +
                "</td>" +
                "<td>" +
                "<a href='#' data-inline='true' data-mini='true' data-role='button' "+
                    "class='btnDelete' data-record-index='" + i + "'>Delete</a>" +
                "</td>" +
            "</tr>"
            );
        }
