...
$(document).on("pagecreate", "#pageNewRecordForm", function () {
    $("#frmNewRecordForm").submit(function (e) {
        e.preventDefault();
        ...
        $.mobile.changePage("#pageRecords"); //returning to records
        return false;
    });
});
...
* and that the date they have entered is not in the future
*/
...
//Load previous records, if they exist
    if (tbRecords != null) {
        ...
        $("#tblRecords").html(
            ...
            "   <tr>" +
            "     <th>Date</th>" +
            "     <th><abbr title='Blood Pressure'>BP (mmHg)</abbr></th>" +
            "     <th>Edit / Delete</th>" +
            " </tr>" +
            "</thead>" +
            "<tbody>" +
        ...
        //data-ajax='false' is a failsafe for any errors that occur when loading forms
        for (var i = 0; i < tbRecords.length; i++) {
            var rec = tbRecords[i];
            $("#tblRecords tbody").append("<tr>" +
                "<td>" + rec.Date + "</td>" +
                "<td>" + rec.Systolic + "/" + rec.Diastolic + "</td>" +
                "<td><a data-inline='true' data-mini='true' data-role='button' href='#pageNewRecordForm' data-ajax='false' onclick = 'callEdit(" + i + ")' data-icon='edit' data-iconpos='notext'></a>" +
                " <a data-inline='true' data-mini='true' data-role='button' href='#' data-ajax='false' onclick='callDelete(" + i + ")' data-icon='delete' data-iconpos='notext'></a></td>" +
                "</tr>");
        }

        $('#tblRecords [data-role="button"]').button();
        // 'Refresh' the buttons. Without this the delete/edit buttons won't appear
    }
    else {
        $("#tblRecords").html("");
    }
